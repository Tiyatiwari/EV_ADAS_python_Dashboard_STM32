...
void EV_ReadADC(EV_HandleTypeDef *ev)
{
    ...
    adc_buf[ADC_RANK_SOC]   = Read_ADC_Channel(ADC_CHANNEL_2); // <-- ADD THIS LINE!
    adc_buf[ADC_RANK_TEMP]  = Read_ADC_Channel(ADC_CHANNEL_3);

    ev->accel_pedal = CLAMP((adc_buf[ADC_RANK_ACCEL] / 4095.0f) * 100.0f, 0.0f, 100.0f);
    ev->brake_pedal = CLAMP((adc_buf[ADC_RANK_BRAKE] / 4095.0f) * 100.0f, 0.0f, 100.0f);
    ev->motor_temp  = CLAMP((adc_buf[ADC_RANK_TEMP]  / 4095.0f) * 120.0f, 0.0f, 120.0f);

    /* Seed the SOC on the very first frame if it hasn't been set yet */
    static uint8_t first_boot = 1;
    if (first_boot) {
        ev->soc = CLAMP((adc_buf[ADC_RANK_SOC] / 4095.0f) * 100.0f, 0.0f, 100.0f);
        first_boot = 0;
    }
}/* ─────────────────────────────────────────────────────────────────────────── */
